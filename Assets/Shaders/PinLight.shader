...
// https://github.com/Elringus/BlendModeExample/blob/master/Assets/Shaders/GrabDarken.shader
Shader "Custom/PinLight"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 PinLight (fixed4 target, fixed4 blend)
			{ 
				fixed4 r = (blend > 0.5) * (max(target, 2*(blend-0.5))) + (blend <= 0.5) * (min(target, 2*blend));
				r.a = blend.a;
				return r;
			} 
